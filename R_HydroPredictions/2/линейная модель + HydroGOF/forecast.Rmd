...
ggplot(cm1[cm1$Var1 == 'Absolute_Levels(t+6)',], aes(x=Var2, y=value, fill=value)) + 
  geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle=90))


target = 'Absolute_Levels(t+6)'
...
```{r}
mod6 <- lm(data = train_df[,-1], formula = `Absolute_Levels(t+6)` ~ .)
summary(mod6)
formula(mod6)
coef(mod6)
```
...
```{r}
test_df$pred6 <- predict(mod6, newdata = test_df)

ggplot(test_df, aes(x=Dates)) + geom_line(aes(y=`Absolute_Levels(t+6)`, col='obs')) +
  geom_line(aes(y=pred6, col='mod'))

ggplot(test_df, aes(x=`Absolute_Levels(t+6)`, y=pred6)) + geom_point() + geom_abline() + geom_smooth(method = 'lm')
cor(test_df$pred6, test_df$`Absolute_Levels(t+6)`, use = "complete.obs")
```

предсказание в оперативном режиме
```{r}

val_df$pred6 <- predict(mod6, newdata = val_df)
...
```{r}
save(train_df, test_df, val_df, file = 'model6.RData')
...
gof(sim = test_df$pred6, obs = test_df$`Absolute_Levels(t+6)`)
ggof(sim = test_df$pred6, obs = test_df$`Absolute_Levels(t+6)`, dates = test_df$Dates)
```
