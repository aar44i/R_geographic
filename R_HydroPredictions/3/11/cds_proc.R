...
setwd('C:/Users/gorba/DataSciense/R_geographic/R_HydroPredictions/2/11')
getwd()
...
era_pts <- st_as_sf(coords, coords = c('x','y'), crs = 4326)
mapview(era_pts) + mapview(shp, native.crs = T)

dat <- as.data.frame(extract(br * 1000, shp, cellnumbers=T)) 
