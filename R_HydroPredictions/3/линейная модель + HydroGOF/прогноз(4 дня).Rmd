...
ggplot(cm1[cm1$Var1 == 'Absolute_Levels(t+4)',], aes(x=Var2, y=value, fill=value)) + 
  geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle=90))


target = 'Absolute_Levels(t+4)'
...
```{r}
mod4 <- lm(data = train_df[,-1], formula = `Absolute_Levels(t+4)` ~ .)
summary(mod4)
formula(mod4)
coef(mod4)
```
...
```{r}
test_df$pred4 <- predict(mod4, newdata = test_df)

ggplot(test_df, aes(x=Dates)) + geom_line(aes(y=`Absolute_Levels(t+4)`, col='obs')) +
  geom_line(aes(y=pred4, col='mod'))

ggplot(test_df, aes(x=`Absolute_Levels(t+4)`, y=pred4)) + geom_point() + geom_abline() + geom_smooth(method = 'lm')
cor(test_df$pred4, test_df$`Absolute_Levels(t+4)`, use = "complete.obs")
```

предсказание в оперативном режиме
```{r}

val_df$pred4 <- predict(mod4, newdata = val_df)
...
```{r}
save(train_df, test_df, val_df, file = 'model4.RData')
...
gof(sim = test_df$pred4, obs = test_df$`Absolute_Levels(t+4)`)
ggof(sim = test_df$pred4, obs = test_df$`Absolute_Levels(t+4)`, dates = test_df$Dates)
```
