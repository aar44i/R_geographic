...
emissions = emissions %>% # ѕервый конвейер манипул€ций дл€ выбросов
  mutate(okrug = if_else(str_detect(emissions$Region, 'федеральный округ'), Region, NULL)) %>% 
  fill(okrug) %>% 
  filter(!str_detect(emissions$Region, '‘едераци€|федеральный округ'))  %>% 
  pivot_longer(cols = 2:9, names_to = "year", values_to = "value")
  
  
capture = capture %>% # ¬торой конвейер манипул€ций дл€ улавоивани€
  mutate(okrug = if_else(str_detect(capture$Region, 'федеральный округ'), Region, NULL)) %>% 
  fill(okrug) %>% 
  filter(!str_detect(capture$Region, '‘едераци€|федеральный округ'))  %>% 
  pivot_longer(cols = 2:9, names_to = "year", values_to = "value")
